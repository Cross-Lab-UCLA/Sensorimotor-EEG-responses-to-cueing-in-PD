function PSD = getPSD_moreAffected(EEG)
%% getPSD
...
for ic_idx = 1:size(EEG.icaact,1)
    ...
    % get HS idxexes
    all_idx = find(~cellfun(@isempty, regexp({event.type}, '^MoreAffected_.*HS1.*$')));
    [PSD, freqs] = extractPSD(PSD, ic_idx, ic_data, all_idx,...
        event, srate, f_range, settings, 'walk_all');

    walk1_idx = find(~cellfun(@isempty, regexp({event.type}, '^MoreAffected_.*HS1_walk-run1.*$')));
    ...
    walk2_idx = find(~cellfun(@isempty, regexp({event.type}, '^MoreAffected_.*HS1_walk-run2.*$')));
    ...
    audi1_idx = find(~cellfun(@isempty, regexp({event.type}, '^MoreAffected_.*HS1_walkAuditory-run1.*$')));
    ...
    audi2_idx = find(~cellfun(@isempty, regexp({event.type}, '^MoreAffected_.*HS1_walkAuditory-run2.*$')));
    ...
    visu1_idx = find(~cellfun(@isempty, regexp({event.type}, '^MoreAffected_.*HS1_walkVisual-run1.*$')));
    ...
    visu2_idx = find(~cellfun(@isempty, regexp({event.type}, '^MoreAffected_.*HS1_walkVisual-run2.*$')));
