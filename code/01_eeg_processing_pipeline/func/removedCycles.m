function [EEG, T] = removedCycles(EEG)
% Find epoch data that deviates from the median and remove them
%
% T = tracking table
%%
% check if EEG set is for moreAffected side analysis or not
if contains(EEG.filepath,'moreAffected')
    disp('processing more affected side analysis')
    HS1_idx = find(~cellfun('isempty', regexp({EEG.event.type}, '^MoreAffected_.*_HS1.*$')));
    rhs1 = round([EEG.event(HS1_idx).latency]);
    rhs = unique(rhs1);
else
    rhs1 = round([EEG.event(contains({EEG.event.type},'R_HS1')).latency]);
    rhs2 = round([EEG.event(contains({EEG.event.type},'R_HS2')).latency]);
...
if isempty(rm_idx)
    ...
    T.total_pnts = EEG.pnts;
    T.outlier_pnts_removed = sum(diff(rm_array')) + length(diff(rm_array'));
    T.pnt_removed_percent = T.outlier_pnts_removed/EEG.pnts * 100;
